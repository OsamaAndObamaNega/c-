...
std::string UserName(std::string username, const std::string &basePath) {
    ...
    try {
        ...
        std::string fileName = username + "_DT.txt"; // File name format
        std::ofstream outFile(fs::path(basePath) / username / fileName);
...
int main()
{
    std::string user_name, Gmail, Password;
    std::string path = "bbbb"; // in future this will be atomated so if it can't find the path then this will make a a folder for this 

    auto gmail_password_returned = Gmail_Password(Gmail, Password);
    user_name = UserName(user_name,path);
...
// void createFileInDirectory(const std::string& path, const std::string& user_name) {
//     // Create the directory if it doesn't exist
//     std::filesystem::create_directories(path);

//     // Construct the full file path
//     std::string file_name = path + "/" + user_name + "_DT.txt";
...
//         file << "This is a file for user: " << user_name << std::endl;
...
//     createFileInDirectory(path, user_name);
